...
## Plans and spec files

When creating a plan, spec, task breakdown, or any intermediate planning document, **always write it to `.memory/` inside the repository root** — never to the project root, `src/`, or any other directory.

```
.memory/
├── plan-country-mode.md
├── spec-historical-api.md
└── ...
```

`.memory/` is git-ignored and treated as a scratch space for agent working files. It keeps the repo root clean and ensures planning documents don't accidentally get committed or interfere with source files.
